...
class keyboard:  # 7 white keys and 5 black ones
    ...
    def __init__(self, masterframe, keys, directory):
        ...
        # directory of white keys
        self.listofkeys = [str(directory)+x+'.wav' for x in self.whitenames]
        # directory of black keys
        self.listofblackkeys = [str(directory)+x+'.wav' for x in self.blacknames]
        # which keyboard key plays white buttons
        self.whitebutt = ['a', 's', 'd', 'f', 'g', 'h', 'j']
...
class assoct:
    def __init__(self, masterframe, lok, directory):  # lok = list of keys, mod 12=0 and put in dict of octlists
        ...
        self.keyboard = keyboard(masterframe=self.masterframe,
                                 keys=self.octlist[self.octkeys[-1]//2], directory=self.directory)
    ...
    def octaveup(self):
        if self.schluessel == self.schlmax:
            ...
            self.schluessel += 1
            self.keyboard = keyboard(masterframe=self.masterframe,
                                     keys=self.octlist[self.schluessel], directory=self.directory)
            self.keyboard.pack()
